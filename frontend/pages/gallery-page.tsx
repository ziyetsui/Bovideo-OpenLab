...
const Residual = ({ model, hidden, title }: Readonly<{ model: FrontendGalleryModel; hidden: boolean; title: string }>) => {
  ...
  return <section className="family-residual" data-slot="residual" aria-labelledby="gallery-residual" hidden={hidden}>
    <h2 id="gallery-residual">{title}</h2>
    {items.length === 0
...
export const GalleryPage = ({ model }: Readonly<{ model: FrontendGalleryModel }>) => {
  ...
  return <div className="page-family page-family--gallery">
    ...
    <section className="family-search" data-slot="search" aria-label={messages.chrome.searchPrompts}>
      <label htmlFor="gallery-search">{messages.chrome.searchPrompts}</label>
    ...
    <Facet model={model} slot="use_cases" title={messages.chrome.useCases} selected={selectedByAxis.use_cases ?? []} onToggle={(nodeRef, pressed) => toggleFacet('use_cases', nodeRef, pressed)} />
    <Facet model={model} slot="styles" title={messages.chrome.styles} selected={selectedByAxis.styles ?? []} onToggle={(nodeRef, pressed) => toggleFacet('styles', nodeRef, pressed)} />
    <Facet model={model} slot="subjects" title={messages.chrome.subjects} selected={selectedByAxis.subjects ?? []} onToggle={(nodeRef, pressed) => toggleFacet('subjects', nodeRef, pressed)} />
    ...
    <Shelf model={model} slot="subject_band" title={messages.chrome.subjectBand} hidden={filterActive} />
    <Residual model={model} hidden={filterActive} title={messages.chrome.residualState} />
    <Shelf model={model} slot="related" title={messages.chrome.related} hidden={filterActive} />
    {model.previous_page === null && model.next_page === null ? null : <nav className="family-pagination" data-slot="pagination" aria-label="Gallery pagination">
      {model.previous_page === null ? null : <a rel="prev" href={model.previous_page}>{messages.chrome.previousPage}</a>}
      {model.next_page === null ? null : <a rel="next" href={model.next_page}>{messages.chrome.nextPage}</a>}
    </nav>}
