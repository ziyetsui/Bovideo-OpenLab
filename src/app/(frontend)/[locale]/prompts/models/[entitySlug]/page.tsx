...
const readProjection = async (locale: string, entitySlug: string) => {
  ...
  const projection = await resolveFrontendRoute({ locale: parsed.data, route: `/${parsed.data}/prompts/models/${entitySlug}`, family: 'entity' })
  return projection?.page.page_type === 'entity' && projection.page.entity_kind === 'model' && projection.page.entity_slug === entitySlug ? projection : undefined
}
...
export default async function ModelEntityRoute({ params }: { params: Promise<Params> }) {
  const resolved = await params
  const projection = await readProjection(resolved.locale, resolved.entitySlug)
  if (projection === undefined) notFound()
