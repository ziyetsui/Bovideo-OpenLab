...
const readProjection = async (locale: string, requestedPage: number) => {
  ...
  const projection = await resolveFrontendRoute({ locale: parsed.data, route: `/${parsed.data}/prompts/image/page/${requestedPage}`, family: 'gallery' })
...
export default async function ImageGalleryPageRoute({ params }: { params: Promise<Params> }) {
  const resolved = await params
  const projection = await readProjection(resolved.locale, Number(resolved.page))
  if (projection === undefined) notFound()
