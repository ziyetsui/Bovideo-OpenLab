...
const readProjection = async (locale: string, requestedPage: number) => {
  ...
  const projection = await resolveFrontendRoute({ locale: parsed.data, route: `/${parsed.data}/prompts/image`, family: 'gallery' })
  return projection?.page.page_type === 'gallery' && projection.page.media_type === 'image' && projection.page.page === requestedPage ? projection : undefined
}
...
export default async function ImageGalleryRoute({ params, searchParams }: { params: Promise<Params>; searchParams: Promise<{ page?: string }> }) {
  const query = await searchParams
  const projection = await readProjection((await params).locale, query.page === undefined ? 1 : Number(query.page))
  if (projection === undefined) notFound()
